...
export function createOrder(state,user,body){const type=body.type||'table';
      ...
      for(const raw of body.lines){
        ...
        const selected=requestedAddons.map(a=>{const found=(state.addons||[]).find(x=>x.id===a?.id&&x.available);return found?{id:found.id,name_en:found.name_en,name_ar:found.name_ar,price_lbp:Number(found.price_lbp||0),quantity:Math.max(1,Math.floor(Number(a?.quantity||1)))}:null}).filter(Boolean);
