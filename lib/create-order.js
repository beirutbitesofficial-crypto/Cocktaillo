...
export function createOrder(state,user,body){const type=body.type||'table';if(!Array.isArray(body.lines)||!body.lines.length)throw new Error('Order items are required.');
      ...
      for(const raw of body.lines){
        ...
        const requestedAddons=item.allow_addons&&Array.isArray(raw.addons)?raw.addons:[];
        const selected=requestedAddons.map(a=>{const found=(state.addons||[]).find(x=>x.id===a?.id&&x.available);const quantity=Number(a?.quantity||1);if(!Number.isSafeInteger(quantity)||quantity<1||quantity>999)throw new Error('Invalid add-on quantity.');return found?{id:found.id,name_en:found.name_en,name_ar:found.name_ar,price_lbp:Number(found.price_lbp||0),quantity}:null}).filter(Boolean);
