...
async function initialState(){
  ...
  return {
    ...
    settings:{business_name:'Cocktaillo',exchange_rate:89500,receipt_footer:'Thank you for visiting Cocktaillo',language:'en',theme:'light'},
    ...
    shifts:[], orders:[], receipts:[], tickets:[], inventory:[], recipes:[], expenses:[], reservations:[], audit:[], next_order_number:1
  };
}
function migrate(state){
  ...
  state.settings={business_name:'Cocktaillo',exchange_rate:89500,receipt_footer:'Thank you for visiting Cocktaillo',language:'en',theme:'light',...(state.settings||{})};
  return state;
}
async function readUnlocked(){
  try { return migrate(JSON.parse(await readFile(file,'utf8'))); } catch { const state=await initialState(); await writeUnlocked(state); return state; }
}
