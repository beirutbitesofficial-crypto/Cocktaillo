...
const extraMenu = [
 ['salad-fruit','Fruit Salad','سلطة فواكه','Salads',600,'bar'],['salad-fruit-nutella','Fruit Salad with Nutella','سلطة فواكه مع نوتيلا','Salads',700,'bar'],['salad-fruit-nuts','Fruit Salad with Nuts','سلطة فواكه مع مكسرات','Salads',700,'bar'],['salad-fruit-cream-honey-nuts','Fruit Salad with Cream, Honey & Nuts','سلطة فواكه مع قشطة وعسل ومكسرات','Salads',800,'bar'],['hookah-two-apples-fakher','Two Apples - Fakher','تفاحتين - فاخر','Hookah',700,'hookah'],['hookah-two-apples-nakhla','Two Apples - Nakhla','تفاحتين - نخلة','Hookah',700,'hookah'],['hookah-two-apples-mix','Two Apples - Mix','تفاحتين - ميكس','Hookah',700,'hookah'],['hookah-lemon-mint','Lemon & Mint','ليمون ونعنع','Hookah',700,'hookah'],['hookah-grape-mint','Grape & Mint','عنب ونعنع','Hookah',700,'hookah'],['hookah-love','Love','لوف','Hookah',700,'hookah'],['hookah-head-change','Head Change','تغيير راس','Hookah',388,'hookah']
...
const defaultSettings={business_name:'Cocktaillo',exchange_rate:89500,receipt_footer:'Thank you for visiting Cocktaillo',language:'en',theme:'light',print_agent_url:'http://127.0.0.1:17483',customer_printer_name:'Customer Receipt',kitchen_printer_name:'Kitchen Printer',bar_printer_name:'Bar Printer',hookah_printer_name:'HOOKAH'};
...
async function initialState(){const passwordManager=await bcrypt.hash(firstBootManagerPassword(),10);return {version:10,users:[{id:'u-manager',name:'Alex Daher',username:'manager',password_hash:passwordManager,role:'manager',active:true}],settings:{...defaultSettings},tables:Array.from({length:12},(_,i)=>({id:`table-${i+1}`,name:`Table ${i+1}`,capacity:[2,4,4,2,6,4,2,8,4,2,4,6][i],status:'available'})),categories:['Dessert',...beverageCategories,'Salads','Hookah'],subcategories:[...dessertSubcategories,...beverageSubcategories],menu,addons,shifts:[],orders:[],receipts:[],tickets:[],print_jobs:[],inventory:[],recipes:[],expenses:[],purchases:[],refunds:[],reservations:[],audit:[],payment_requests:[],next_order_number:1}}
function migrate(state){
  const oldVersion=Number(state.version||0);
  state.version=10;
  for(const k of ['receipts','recipes','inventory','expenses','purchases','refunds','reservations','audit','print_jobs','orders','payment_requests'])state[k]=Array.isArray(state[k])?state[k]:[];
  ...
  state.menu=(state.menu||[]).map(i=>{
    const next=['Crepe','Waffle','Pancakes','Pancake','Cold Dessert','Ice Cream','Merry Cream'].includes(i.category)?{...i,category:'Dessert',subcategory:i.category==='Pancakes'?'Pancake':i.category}:{...i,subcategory:typeof i.subcategory==='string'?i.subcategory:''};
    if(next.category==='Hookah'&&!next.deleted)next.station='hookah';
    return next;
  });
  ...
  for(const item of incoming){
    ...
    if(current){
      ...
      if(!['bar','kitchen','service','hookah'].includes(current.station))current.station=item.station;
...
async function acquireFileLock(){await mkdir(path.dirname(file),{recursive:true});const started=Date.now();while(true){try{const handle=await open(lockFile,'wx');await handle.writeFile(`${process.pid}:${Date.now()}`);return async()=>{try{await handle.close()}finally{await unlink(lockFile).catch(e=>{if(e?.code!=='ENOENT')throw e})}}}catch(e){if(e?.code!=='EEXIST')throw e;try{const info=await stat(lockFile);const owner=Number((await readFile(lockFile,'utf8')).split(':')[0]);let alive=true;try{process.kill(owner,0)}catch(error){alive=error.code!=='ESRCH'}if(!alive&&Date.now()-info.mtimeMs>30000){await unlink(lockFile).catch(err=>{if(err?.code!=='ENOENT')throw err});continue}}catch(err){if(err?.code==='ENOENT')continue;throw err}if(Date.now()-started>=10000)throw new Error('POS data file is busy. Please retry the operation.');await wait(50)}}}
export async function readState(){try{await access(file);return await readUnlocked()}catch(e){if(e?.code!=='ENOENT')throw e;return mutateState(state=>state)}}
...
export const orderTotal=(order,rate=89500)=>{const exchangeRate=Number(rate);if(!Number.isFinite(exchangeRate)||exchangeRate<=0)throw new Error('Exchange rate must be a positive number.');const usd=order.lines.reduce((s,l)=>s+l.price_cents*l.quantity,0);const lbp=(order.lines||[]).reduce((s,l)=>s+(Array.isArray(l.addons)?l.addons:[]).reduce((a,x)=>a+Number(x.price_lbp||0)*Math.max(1,Number(x.quantity||1))*Math.max(1,Number(l.quantity||1)),0),0);const before=usd+Math.round((lbp/exchangeRate)*100);const d=order.discount||null;let discount_cents=0;if(d?.type==='percent')discount_cents=Math.round(before*Math.min(100,Math.max(0,Number(d.value||0)))/100);if(d?.type==='fixed')discount_cents=Math.min(before,Math.round(Math.max(0,Number(d.value||0))*100));return {usd_cents:usd,lbp,subtotal_equivalent_cents:before,discount_cents,total_equivalent_cents:Math.max(0,before-discount_cents)}};
