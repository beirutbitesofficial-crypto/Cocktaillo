...
const sessionSecret=configuredSecret||`${crypto.randomUUID()}-${crypto.randomUUID()}`;
if(process.env.NODE_ENV==='production'&&!configuredSecret){console.warn('[Cocktaillo] POS_SESSION_SECRET is not configured. A secure temporary session secret is being used, so users will need to sign in again after a server restart.');}
