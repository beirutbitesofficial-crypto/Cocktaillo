...
export async function autoPrintCustomerReceipt(receiptId){const bundle=await serverPost({action:'create',receipt_id:receiptId,mode:'automatic'});if(await localServiceReady())return sendCustomerToAgent(bundle);emit({status:'queued',jobId:bundle.job?.id||null,receiptId,message:`Receipt #${bundle.job?.order_number||''} queued for the cashier printer.`});return bundle}
export async function reprintCustomerReceipt(receiptId){const bundle=await serverPost({action:'create',receipt_id:receiptId,mode:'reprint'});if(await localServiceReady(true))return sendCustomerToAgent(bundle);emit({status:'queued',jobId:bundle.job?.id||null,receiptId,message:`Receipt #${bundle.job?.order_number||''} queued for the cashier printer.`});return bundle}
