...
async function localServiceReady(force=false){const settings=getLocalPrintSettings();if(!settings.token.trim())return false;const now=Date.now();if(!force&&now-lastHealthAt<12000)return lastHealthOk;lastHealthAt=now;try{await agentRequest('/health',{},settings);lastHealthOk=true}catch{lastHealthOk=false}return lastHealthOk}
...
async function sendCustomerToAgent(bundle,claimed=false){
  const {job,receipt,should_print}=bundle;if(!should_print){if(job?.status==='printed')emit({status:'printed',jobId:job.id,receiptId:job.receipt_id,message:`Receipt #${job.order_number} already printed.`});return bundle}
  const settings=getLocalPrintSettings();if(!claimed)await serverPost({action:'status',job_id:job.id,status:'printing'});
  try{await agentRequest('/print',{method:'POST',body:JSON.stringify({job_id:job.id,destination:'customer',printer_name:settings.customerPrinter||bundle.printer_name,receipt})},settings);await serverPost({action:'status',job_id:job.id,status:'printed'});emit({status:'printed',jobId:job.id,receiptId:job.receipt_id,message:`Customer receipt #${job.order_number} printed.`});return bundle}
  catch(e){if(!claimed)await serverPost({action:'status',job_id:job.id,status:'failed',error:e.message}).catch(()=>{});throw Object.assign(e,{jobId:job.id,receiptId:job.receipt_id})}
}
export async function autoPrintCustomerReceipt(receiptId){const bundle=await serverPost({action:'create',receipt_id:receiptId,mode:'automatic'});if(await localServiceReady()){try{return await sendCustomerToAgent(bundle)}catch(e){emit({status:'failed',jobId:bundle.job?.id||null,receiptId,message:`Receipt #${bundle.job?.order_number||''} is queued for retry`,error:e.message});return bundle}}emit({status:'queued',jobId:bundle.job?.id||null,receiptId,message:`Receipt #${bundle.job?.order_number||''} queued for the cashier printer.`});return bundle}
export async function reprintCustomerReceipt(receiptId){const bundle=await serverPost({action:'create',receipt_id:receiptId,mode:'reprint'});if(await localServiceReady(true)){try{return await sendCustomerToAgent(bundle)}catch(e){emit({status:'failed',jobId:bundle.job?.id||null,receiptId,message:`Receipt #${bundle.job?.order_number||''} is queued for retry`,error:e.message});return bundle}}emit({status:'queued',jobId:bundle.job?.id||null,receiptId,message:`Receipt #${bundle.job?.order_number||''} queued for the cashier printer.`});return bundle}
...
export async function retryPrintJob(jobId){const bundle=await serverPost({action:'retry',job_id:jobId});if(!(await localServiceReady(true))){emit({status:'queued',jobId,message:'Print job queued for the cashier printer.'});return bundle}try{return bundle.job?.destination==='bar'?await sendBarToAgent(bundle):await sendCustomerToAgent(bundle)}catch(e){emit({status:'failed',jobId,message:'Printing needs attention',error:e.message});throw e}}

async function processPrintQueue(){
  if(workerBusy)return;if(!(await localServiceReady()))return;workerBusy=true;
  try{const bundle=await serverPost({action:'claim-next'});if(!bundle?.job)return;try{if(bundle.job.destination==='bar')await sendBarToAgent(bundle);else await sendCustomerToAgent(bundle,true)}catch(e){await serverPost({action:'status',job_id:bundle.job.id,status:'failed',error:e.message}).catch(()=>{});emit({status:'failed',jobId:bundle.job.id,message:`Print job #${bundle.job.order_number} is queued for retry`,error:e.message})}}
  catch{}finally{workerBusy=false}
}
export function startCentralPrintWorker(){if(typeof window==='undefined'||workerTimer)return()=>{};void processPrintQueue();workerTimer=window.setInterval(()=>void processPrintQueue(),1800);return()=>{if(workerTimer){clearInterval(workerTimer);workerTimer=null}}}

export function installPrintBridgeInterceptor(){if(installed||typeof window==='undefined')return()=>{};installed=true;nativeFetch=window.fetch.bind(window);window.fetch=async function(input,init){const response=await nativeFetch(input,init);try{const url=typeof input==='string'?input:input?.url||'';if(response.ok&&init?.method?.toUpperCase()==='POST'){const cloned=response.clone();cloned.json().then(d=>{if((url.includes('/api/actions')||url.includes('/api/split-pay'))&&d?.receipt?.id)void autoPrintCustomerReceipt(d.receipt.id).catch(()=>{})}).catch(()=>{})}}catch{}return response};return()=>{if(nativeFetch){window.fetch=nativeFetch;installed=false}}}
