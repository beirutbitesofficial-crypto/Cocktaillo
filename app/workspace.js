...
export default function Workspace({initialUser}){
  ...
  useEffect(()=>{if(language!=='ar'||!data)return;let stop=()=>{};const frame=requestAnimationFrame(()=>{stop=observeArabicUi(document.querySelector('.app'))});return()=>{cancelAnimationFrame(frame);stop()}},[language,data,tab]);
