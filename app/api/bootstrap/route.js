...
export async function GET(){
 ...
 if(user.role==='cashier')return NextResponse.json({...base,shifts:s.shifts.filter(x=>x.user_id===user.id),tickets:s.tickets},{headers:{'Cache-Control':'no-store'}});
 const paid=s.orders.filter(o=>o.status==='paid');
 return NextResponse.json({...base,shifts:s.shifts,tickets:s.tickets,users:s.users.map(publicUser),inventory:s.inventory,expenses:s.expenses,reservations:s.reservations,reports:{sales_cents:paid.reduce((a,o)=>a+orderTotal(o,s.settings.exchange_rate).total_equivalent_cents,0),orders:paid.length}},{headers:{'Cache-Control':'no-store'}})
}
