...
try{
 ...
 const together=await Promise.all([post('/api/orders',{...b,request_id:'parallel-w1',table_id:'table-2',lines:[{menu_item_id:'test-oreo',quantity:2,note:'split test'}]},w1),post('/api/orders',{...b,request_id:'parallel-w2',table_id:'table-3'},w2)]);assert.notEqual(together[0].d.order.id,together[1].d.order.id);
 ...
 const splitSource=afterParallel.orders.find(o=>o.id===together[0].d.order.id),splitRequest='split-pay-1',splitBody={action:'pay_selected_items',payment_request_id:splitRequest,order_id:splitSource.id,items:[{line_id:splitSource.lines[0].id,quantity:1}],usd:7,lbp:0,change_currency:'USD'};
 const splitPaid=await post('/api/actions',splitBody,c),splitRetry=await post('/api/actions',splitBody,c);assert.equal(splitPaid.d.receipt.id,splitRetry.d.receipt.id);assert.equal(splitPaid.d.receipt.totals.total_equivalent_cents,700);assert.equal(splitPaid.d.receipt.items[0].quantity,1);assert.equal(splitPaid.d.partial,true);
 const afterSplit=await(await fetch(base+'/api/bootstrap',{headers:{cookie:c}})).json(),remaining=afterSplit.orders.find(o=>o.id===splitSource.id&&o.status==='open'),paidSplitOrder=afterSplit.orders.find(o=>o.id===splitPaid.d.receipt.order_id&&o.status==='paid');assert.equal(remaining.lines[0].quantity,1);assert.equal(paidSplitOrder.lines[0].quantity,1);assert.equal(remaining.table_id,'table-2');
 ...
 console.log('HTTP smoke passed: waiter ownership, retry deduplication, partial table item payment, printer claim, duplicate payment and shift close.');
